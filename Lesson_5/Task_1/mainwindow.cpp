...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->maxPriceInput, &QLineEdit::textChanged, this,
            &MainWindow::updateMaxPriceFilter);
    connect(ui->carTypeList, &QComboBox::currentTextChanged, this,
            &MainWindow::updateCarTypeFilter);
    connect(ui->carManufacturerList, &QComboBox::currentTextChanged, this,
...
void MainWindow::setUpFilters() {
    ...
    for (int index = 0; index < MAX_CARS_COUNT; index++) {
        ...
        QString carManufacturer =
            QString::fromStdString(currentCar.getManufacturer());
...
void MainWindow::updateCarList() {
    ...
    for (int index = 0; index < MAX_CARS_COUNT; index++) {
        ...
        if (carManufacturerFilter != "All" &&
            carManufacturer != carManufacturerFilter)
            continue;

        string tempBuffer = "";
        tempBuffer +=
            currentCar.getManufacturer() + " " + currentCar.getModelRange() + " ";
        tempBuffer += "[Type: " + currentCar.getType() + "] ";
        tempBuffer += "[Price: " + to_string(currentCar.getPrice()) + "] ";
        tempBuffer += "[Color: " + currentCar.getColor() + "] ";
        tempBuffer +=
            "[Seats Count: " + to_string(currentCar.getSeatsCount()) + "]";
